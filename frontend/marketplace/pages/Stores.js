...
const Stores = () => {
  ...
  const handleCategory = React.useCallback((data) => {
    setCategory(data)
    history.push(`?search=${search ? search : "" }&category=${data}`)
  }, [category])

const handleSearch = React.useCallback((e) => {
  history.push(`?search=${e.target.value}&category=${category}`)
...
const handlePagination = (value) => {
  ...
  const handleSubmit = (e) => {
   e.preventDefault()
   dispatch(getBrands(0, location.search)) 
  }
       ...
       <div className="ml-36 mobile:ml-0">
            <div className="bar">
               <div className="grid grid-cols-2 max-w-sm">
                    ...
                    <NavLink exact to="/marketplace" className="p-2 border-b-2 justify-center font-bold text-center cursor-pointer flex "  activeClassName="border-Blue text-Blue" >
